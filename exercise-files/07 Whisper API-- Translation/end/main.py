...
if submit_button and uploaded_file is not None:
    with st.spinner("Transcribing..."):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            original_transcript = speech_to_text(temp_file_path) #transcription
            translated_transcript = speech_to_translation(temp_file_path) #translation
            ...
            st.divider()
            st.markdown(f":blue `{original_transcript}`") #can read original transcript
            st.divider()
            st.markdown(f":green `{translated_transcript}`") #and read translation in target language
            st.audio(temp_file_path) #listen to translation
